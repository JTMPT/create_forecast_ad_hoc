...
# קטע קוד שלקחתי מחלוקת אזורים סטטסיטים
...
bld_point=gpd.sjoin(bld_point,EMP_kibolet_by_taz[['ID','Taz_num','geometry']])
...
# ### יצירת מאה אחוז לכל מתחם תעסוקה

# In[97]:


sum_bld_m3=pd.pivot_table(bld_point.drop(columns='centroid'),index=['Taz_num','ID'],aggfunc=sum)[['bld_m3']].reset_index()


# In[98]:


sum_bld_m3_by_ID=sum_bld_m3.pivot_table(index='ID',aggfunc=sum)[['bld_m3']].rename(columns={'bld_m3':'bld_m3_ID'}).reset_index()


# In[99]:


sum_bld_m3=sum_bld_m3.merge(sum_bld_m3_by_ID,on='ID')


# In[101]:


sum_bld_m3['precent_of_stat_data']=sum_bld_m3['bld_m3']/sum_bld_m3['bld_m3_ID']


# In[102]:


sum_bld_m3=sum_bld_m3.loc[(sum_bld_m3['precent_of_stat_data']>0.01)]


# In[103]:


sum_bld_m3_by_ID=sum_bld_m3.pivot_table(index='ID',aggfunc=sum)[['bld_m3']].rename(columns={'bld_m3':'bld_m3_ID_new'}).reset_index()
...
sum_bld_m3['precent_of_stat_data']=sum_bld_m3['bld_m3']/sum_bld_m3['bld_m3_ID_new']
...
col=['Taz_num', 'ID',  'precent_of_stat_data']
...
sum_bld_m3[col].to_excel(r'{}\Monitoring\{}_precent_of_ID_data.xlsx'.format(cwd, file_date),index=False)
...
bld_point.to_excel(r'{}\Monitoring\{}bld_point.xlsx'.format(cwd, file_date),index=False)


# עד כאן קטע קוד שלקחתי מחלוקת אזורים סטטסיטים
